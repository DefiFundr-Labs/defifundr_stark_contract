...
use defifundr_contract::verify_sig::{
    IVerifySignatureSafeDispatcher, IVerifySignatureDispatcher, IVerifySignatureDispatcherTrait,
    IVerifySignatureSafeDispatcherTrait,
};

const TEST_MESSAGE: felt252 = 0x65ad64cab8a5e6aca149cb913a31ad2d0129a0c81fe20c33c2d5f1892215529;

const VALID_SIGNATURE_R: felt252 =
    0x7cbf320dabe3be212384b74213ffff100e75920513e74558ceec110d041d133;
const VALID_SIGNATURE_S: felt252 =
    0x2a28596801bae683197812ab8072489289b4230d51a0dcd1b6b1848c234472e;
...
fn test_successful_verification() {
    ...
    let success = dispatcher
        .verify_signature(user_address, TEST_MESSAGE, VALID_SIGNATURE_R, VALID_SIGNATURE_S);
...
fn test_invalid_signature() {
    ...
    let verified = dispatcher
        .verify_signature(user_address, TEST_MESSAGE, INVALID_SIGNATURE_R, INVALID_SIGNATURE_S);
...
fn test_multiple_users() {
    ...
    let user1_address = 0x3c10a541a68dd7b4f9be06c5ccac0f2d91e2810c5cfe460370e3cee25afce56
        .try_into()
        .unwrap();
    let user2_address = 0xbc774f8ca1bbe71728cd661ad4515d66dca540bd697788e48787f970900a0b
        .try_into()
        .unwrap(); // Replace with a different public key

    let dispatcher = IVerifySignatureDispatcher { contract_address };

    // Verify first user
    start_cheat_caller_address(contract_address, user1_address);
    let success1 = dispatcher
        .verify_signature(user1_address, TEST_MESSAGE, VALID_SIGNATURE_R, VALID_SIGNATURE_S);
    assert(success1, 'User1 verification failed');
    stop_cheat_caller_address(contract_address);

    // Verify second user
    start_cheat_caller_address(contract_address, user2_address);
    let success2 = dispatcher
        .verify_signature(
            user2_address,
            TEST_MESSAGE,
