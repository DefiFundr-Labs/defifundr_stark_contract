...
pub mod VerifySignature {
    ...
    use starknet::{get_caller_address, get_block_timestamp, contract_address::ContractAddress};
    ...
    pub impl VerifySignatureImpl of IVerifySignature<ContractState> {
        fn verify_signature(
            ...
            if is_valid {
                self.emit(AuthenticationSuccess { user_address: claimed_address, timestamp });
                true
            } else {
                self.emit(AuthenticationFailure { claimed_address, timestamp });
                false
            }
        }
